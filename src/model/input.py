...
AGENT_ARN  = os.getenv("AGENT_ARN",  "arn:aws:bedrock-agentcore:ap-south-1:767398019214:runtime/atomicAquaLangGraph_Agent-ADIW4YEOjJ")

# ── File ──────────────────────────────────────────────────────────────
# Usage: python input.py "D:\Customer_data\yourfile.log"
file_path  = sys.argv[1] if len(sys.argv) > 1 else r"D:\Customer_data\caltest.ASCII"
...
# ── Step 4: Read response (tries all known key names) ─────────────────
# ── Step 4: Read response ─────────────────────────────────────────────
result = json.loads(response["response"].read())
print("\nAgent response:", result.get("result", result))

# ── Step 5: Print session ID ──────────────────────────────────────────
